...
1. **P1 release evidence:** native macOS packaging/CI source and portable artifact re-verification exist. Exact-commit Apple-silicon execution, preview-extension runtime observation, and provisioned cross-process app-group proof remain under PAR-007.
...
| Text | **Full** | Editing and rich Unicode DXF/SVG/PNG/PDF output are implemented. An authoritative physical 2x2 basis preserves shear, reflection, and non-uniform affine transforms across canvas, persistence, DXF, SVG, PDF, and PNG. PDF uses deterministic bundled OFL CJK/emoji fonts, exact ToUnicode maps, grapheme-safe fallback, transformed multiline layout, and atomic writes. PAR-017/PAR-018/PAR-020 complete. |
...
| Import routing | **Full** | STCH, ASCII/binary DXF, SVG, PDF, PSD/raster, STEP/STP, OBJ, and STL route correctly. DXF supports BOM-safe UTF-8, legacy declared codepages, Unicode escapes, SPLINE/HATCH, bounded curves, planar OCS projection, and exact mirrored/sheared TEXT bases; packaged ezdxf converts binary transport into a temporary ASCII import intermediary without changing the source. PAR-012 through PAR-016/PAR-020 complete. |
| 2D export | **Full** | Generated DXF/SVG/PDF/PNG, selected-only, measurements, canonical millimetres, fills/holes, rich layout, and full-Unicode PDF embedding work. Unchanged direct/generated DXFs preserve opaque structure through full export and project save/reopen; supported planar edits, safe HATCH similarities, deletions, and new entities/dependencies merge without discarding opaque records. Safety-gated cases use validated model serialization. PAR-012 through PAR-020 complete. |
...
**Status (in progress 2026-07-18):** Native source gate expanded and stale tracker reconciled. R&D audit found and source-fixed CI disk duplication, missing packaged STCH ZIP decoding, shallow round-trip checks, partial nested signing, unpinned Swift deployment target, generic app icon, and evidence paths that trusted producer booleans/hashes without independently parsing DXF semantics. Packaging uses pinned ZIPFoundation source, registered ICNS, macOS 13 target, inside-out all-Mach-O signing, and 90-day native-evidence retention. Packaged acceptance schema 4 exercises STEP plus OBJ/STL/mixed OCCT routing, connected/separate nets, manual seams, tabs, holes, exact STCH source/output hashes, and preserved input/output artifacts. It requires unique $INSUNITS=4 and $MEASUREMENT=1 declarations inside each DXF HEADER and records those values per descriptor. Independent collector parses every retained/new DXF, records dxfUnitEvidence, extracts STCH project.json.dxfDataBase64, and binds its SHA-256 to external unfold and round-trip hashes. Independent reviewer leaves downloaded evidence unchanged, verifies exact file inventory and expected commit/run, re-runs portable semantic checks in temporary copy, and emits hash-bound receipt. Twenty-seven corrupted, tampered, or identity-mismatched bundles cannot produce passed status; validator tests pass 29/29. AUD-021, AUD-022, and AUD-032 remain honestly In progress. Warnings-as-errors build passes; focused collector/delivery contracts pass 32/32; full .NET suite passes 1151/1151; packaged OCCT mesh/import/unfold tests pass 23/23. Exact-commit Apple-silicon execution, preview-extension runtime observation, and provisioned cross-process app-group proof are still required; no native result is claimed from this Windows host. Sign-off template: docs/architecture/PAR-007-macos-native-evidence.md.
...
- Portable reviewer binds expected commit/run independently, checks exact producer inventory, revalidates semantic evidence in an isolated copy, and emits a hash-bound receipt without changing downloaded evidence.
...
- Attach hashes/logs/screenshots plus independent reviewer receipt to evidence doc, then reconcile stale `AUDIT_TASKS.md`. Source presence alone is not packaged evidence.
...
**Priority:** P1  
**Status (completed 2026-07-18):** R2007+ DXF writes and strictly reads UTF-8, including BOM-prefixed files. Transport declarations are discovered only inside one complete HEADER section; duplicate/malformed ACADVER or DWGCODEPAGE declarations fail explicitly, while entity payload lookalikes cannot override decoding. R2000 output stays ASCII using standard Unicode escapes, including surrogate pairs and length-safe XDATA chunks. Legacy imports honor declared codepages such as ANSI_1252; malformed or unsupported declarations fail with actionable errors. Binary DXF routes through the pinned packaged ezdxf runtime into a validated temporary ASCII intermediary; source bytes remain unchanged, temporary output is deleted on success/failure, and the strict low-level parser still rejects raw binary input. The intermediary is model-import transport, not an opaque structure-preserving rewrite. LWPOLYLINE, legacy POLYLINE, ARC, CIRCLE, HATCH, ELLIPSE, and TEXT use arbitrary-axis OCS projection. Projected ARC/CIRCLE geometry becomes polylines when circular metadata would be invalid. TEXT parses groups 40/41/50/51/71 plus OCS into an authoritative physical 2x2 basis. Canvas, bounds/hit testing, affine edits, project JSON, DXF merge validation, and DXF/SVG/PDF/PNG output resolve that basis first. DXF output decomposes every supported nonsingular basis back into exact height, rotation, width, oblique angle, and generation flags; singular or out-of-range bases fail instead of being approximated. Legacy scalar text remains backward compatible.  
...
Open work remains only in PAR-007. Re-run native evidence after any native, packaging, update, geometry-worker, unit-contract, or recent-discovery change.
...
- Final warnings-as-errors verification passed with 0 warnings/errors; full .NET suite passed 1151/1151. PAR-018 Unicode PDF tests passed 3/3, PAR-019 structure-preservation tests passed 46/46, Python unit/export/append contracts passed 7/7, recent-project lifecycle tests passed 9/9, focused collector/delivery contracts passed 32/32, executable native-evidence validation passed 29/29, and combined packaged OCCT mesh/import/unfold Python suites passed 23/23. Native macOS packaging cannot execute on this Windows host and remains explicitly gated by PAR-007.
