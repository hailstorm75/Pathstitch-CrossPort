...
- Schema-5 native evidence retains one self-contained, hash-inventoried root with package ZIP, input/output geometry, STCH archive, acceptance JSON, parsed DXF-unit facts, signing/entitlements, plugin logs/maps/PNGs, runtime STEP fixture, and four nonce-bound app/extension attestations.
- Real preview and thumbnail extension hosts attest nondefault app-group values, fixture hash, bundle/process identity, rendering, and interactive SceneKit camera-control state; packaged app retrieves records through same group container.
